...
use crate::models::game::{CreateGameSchema, UpdateGameSchema, GameModel};
use serde_json::json;
use crate::AppState;
use chrono::Utc;
...
pub async fn get_games(data: web::Data<AppState>) -> impl Responder {
    ...
    if query_result.is_err(){
        let message: &str = "something goes wrong while fetching the games";
        return HttpResponse::InternalServerError().json(json!({"status": "Err", "message": message}));
    }
